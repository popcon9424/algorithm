...
while status != 'finish':
    # 매일 user list를 업데이트
    users = requests.get(url = baseurl + 'users?page=1&size=3000', headers = headers).json()['users'] + requests.get(url = baseurl + 'users?page=2&size=3000', headers = headers).json()['users']
